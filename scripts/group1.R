...
D  <- 8         # mean infectious duration (days)
...
out |>
  ...
  labs(x = "Day", y = "People", colour = NULL,
       title = paste0("Measles-like SIR  (R0 = ", R0, ", D = ", D, " days)")) +
...
bind_rows(
  ...
  labs(x = "Day", y = "Infectious", colour = NULL,
       title = paste0("Measles-like — SIR vs SEIR (latent period 5 days)")) +
  theme_minimal(base_size = 14)
