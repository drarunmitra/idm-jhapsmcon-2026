...
R0 <- 3         # basic reproduction number (early SARS-CoV-2 ancestral)
D  <- 7         # mean infectious duration (days)
...
out |>
  ...
  labs(x = "Day", y = "People", colour = NULL,
       title = paste0("COVID-19-like SIR  (R0 = ", R0, ", D = ", D, " days)")) +
...
bind_rows(
  ...
  labs(x = "Day", y = "Infectious", colour = NULL,
       title = paste0("COVID-19-like — SIR vs SEIR (latent period 5 days)")) +
  theme_minimal(base_size = 14)
